...
const TRANSLATIONS = {
  KR: {
    ...
    stats_defect_val: "0.01",
    stats_defect_label: "최소 결함률",
    stats_defect_sub: "자체 편직으로 실현한 수치",
    explore: "인벤토리 확인하기",
    inventory_title: "Signature",
    inventory_span: "Archive",
    inventory_p: "화성섬유가 보증하는 주력 제품군입니다. 클릭하여 상세 사양을 확인하세요.",
    biz_title: "Business Infrastructure",
    biz_sub: "System Solutions",
    sector_a_title: "편직 / 생지",
    sector_a_p: "32G 고밀도 환편 시스템을 통해 미세한 터치감의 차이를 만드는 자체 공장 직영 라인입니다.",
...
const CountUp = React.memo(({ end, duration = 2000, suffix = "", isVisible }) => {
  ...
  useEffect(() => {
    ...
    const step = (timestamp) => {
      ...
      const currentCount = Math.floor(progress * parseFloat(end));
...
const StatBoxItem = React.memo(({ icon: Icon, label, value, suffix, sub, isVisible, highlight = false }) => (
  <div className={`p-10 border-r border-white/5 last:border-r-0 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} ${highlight ? 'bg-indigo-600/5' : ''}`}>
    <div className="flex items-center space-x-3 mb-6">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${highlight ? 'bg-indigo-600 text-white' : 'bg-white/5 text-indigo-400'}`}>
        {Icon && <Icon size={20} />}
      </div>
      <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">{label}</span>
    </div>
    <div className={`text-4xl md:text-5xl font-black mb-4 tracking-tighter ${highlight ? 'text-indigo-400' : 'text-white'}`}>
      <CountUp end={value} suffix={suffix} isVisible={isVisible} />
    </div>
    <p className="text-slate-500 text-[10px] font-bold tracking-tight break-keep">{sub}</p>
  </div>
));

const VisualPlaceholder = React.memo(({ text, dark = false }) => (
  <div className={`w-full h-full flex items-center justify-center relative overflow-hidden group ${dark ? 'bg-slate-900' : 'bg-slate-100'}`}>
    ...
    <div className="relative z-10 flex flex-col items-center">
      <div className={`w-20 h-[1px] mb-4 transition-all duration-700 group-hover:w-32 ${dark ? 'bg-white/20' : 'bg-slate-300'}`}></div>
      <span className={`text-[10px] font-black tracking-[0.4em] uppercase ${dark ? 'text-white/20' : 'text-slate-300'}`}>{text}</span>
    </div>
  </div>
));
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 leading-relaxed overflow-x-hidden selection:bg-indigo-600 selection:text-white relative">
            ...
            <div className="flex flex-col lg:flex-row gap-20 items-center">
              ...
              <div className="w-full lg:w-1/2">
                ...
                <div className="grid gap-8 mb-12">
                  <div className="p-8 bg-slate-50 border-l-4 border-slate-900 hover:border-indigo-600 transition-colors group cursor-pointer" onClick={() => navigateTo('knitting')}>
                    <h3 className="text-xl font-bold mb-3 flex items-center group-hover:text-indigo-600 transition-colors">
                      {t.sector_a_title}
                      <ArrowUpRight size={16} className="ml-2 opacity-0 group-hover:opacity-100 transition-all translate-y-1 group-hover:translate-y-0" />
                    </h3>
                    <p className="text-slate-500 text-sm leading-relaxed">{t.sector_a_p}</p>
                  </div>
                  <div className="p-8 bg-slate-50 border-l-4 border-slate-900 hover:border-indigo-600 transition-colors group cursor-pointer" onClick={() => navigateTo('processing')}>
                    <h3 className="text-xl font-bold mb-3 flex items-center group-hover:text-indigo-600 transition-colors">
                      {t.sector_b_title}
                      <ArrowUpRight size={16} className="ml-2 opacity-0 group-hover:opacity-100 transition-all translate-y-1 group-hover:translate-y-0" />
                    </h3>
                    <p className="text-slate-500 text-sm leading-relaxed">{t.sector_b_p}</p>
                  </div>
                </div>
      ...
      {view === 'main' && (
        <section className="py-24 md:py-40 bg-slate-50 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-1/3 h-full bg-white/50 backdrop-blur-3xl -skew-x-12 translate-x-1/2"></div>
          <div className="container mx-auto px-6 md:px-10 relative z-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 md:mb-24 gap-8">
              <div className="max-w-2xl">
                <div className="inline-block px-3 py-1 bg-indigo-600 text-white text-[10px] font-black tracking-widest uppercase mb-6 rounded-sm">Archive</div>
                <h2 className="text-5xl md:text-7xl font-black text-slate-900 leading-[0.9] tracking-tighter italic">
                  {t.inventory_title} <span className="text-indigo-600 not-italic">{t.inventory_span}</span>
                </h2>
              </div>
              <p className="max-w-xs text-slate-500 text-sm font-medium leading-relaxed border-l-2 border-slate-200 pl-6">
                {t.inventory_p}
              </p>
            </div>
      ...
      {view === 'about' && (
        <section className="bg-slate-900 text-white min-h-screen pt-40 pb-24 overflow-hidden">
          <div className="container mx-auto px-6 md:px-10">
            {/* About Hero */}
            <div className="flex flex-col xl:flex-row gap-20 items-start mb-40">
              <div className="w-full xl:w-1/2">
                <div className="inline-block px-4 py-1 border border-indigo-500/30 text-indigo-400 text-[10px] font-black tracking-[0.3em] uppercase mb-10 rounded-sm">
                  {t.about_hero_tag}
                </div>
                <h2 className="text-7xl md:text-9xl font-black mb-10 tracking-tighter leading-[0.85] italic">
                  {t.about_hero_h1.split(' ').map((word, i) => (
                    <span key={i} className="block">{word}</span>
                  ))}
                  <span className="text-3xl md:text-4xl not-italic text-white/20 block mt-4 font-light">{t.about_hero_h1_sub}</span>
                </h2>
                <div className="max-w-xl">
                  <p className="text-indigo-400 font-mono text-lg mb-6 leading-tight break-keep">{t.about_hero_motto}</p>
                  <p className="text-slate-400 text-xl leading-relaxed font-medium break-keep">
                    {t.about_hero_sub}
                  </p>
                </div>
              </div>
              <div className="w-full xl:w-1/2 relative aspect-video xl:aspect-square bg-slate-800 rounded-sm overflow-hidden group shadow-[0_0_100px_rgba(79,70,229,0.1)]">
      ...
      <section className="bg-slate-50 py-24 md:py-40 border-t border-slate-100 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 md:px-10 relative z-10">
          <div className="flex flex-col lg:flex-row gap-20 items-stretch">
            ...
            <div className="w-full lg:w-1/2 min-h-[400px] bg-slate-200 rounded-sm overflow-hidden shadow-2xl relative border-8 border-white">
